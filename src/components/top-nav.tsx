    ...
    <nav className="maze-nav">
      <div className="h-full max-w-[1200px] mx-auto px-6 lg:px-10 flex items-center justify-between">
          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`relative px-3.5 py-2 rounded-lg text-[13px] font-semibold tracking-[-0.01em] transition-colors ${
                  isActive
                    ? "text-foreground bg-lime/10"
              ...
              >
                ...
                {link.href === "/review" && pendingCount > 0 && (
                  <span className="absolute -top-1 -right-1 h-[18px] min-w-[18px] px-1 flex items-center justify-center rounded-full bg-lime text-[9px] font-extrabold text-lime-foreground shadow-sm">
                    {pendingCount}
                  </span>
                )}
              </Link>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-muted-foreground font-semibold tracking-wide uppercase" style={{ fontFamily: "var(--font-jakarta), system-ui, sans-serif" }}>
            Active
          </span>
        </div>
      </div>

...
const TOOL_NODES = [
  {
    ...
    angle: -90,
    color: "#10a37f",
    // OpenAI hexagonal flower — scaled to fit inside the circle
    logoPath:
  ...
  },
  {
    ...
    angle: 30,
    color: "#d4a574",
    // Anthropic calligraphic lightning mark
    logoPath:
      "M16.878 10.414l-4.89 8.457a.497.497 0 01-.37.227.473.473 0 01-.408-.157.551.551 0 01-.124-.436l.749-4.838-3.47-.903a.533.533 0 01-.345-.283.538.538 0 01-.022-.45l4.89-8.456a.497.497 0 01.37-.228.473.473 0 01.408.158.551.551 0 01.124.436l-.749 4.837 3.47.903a.533.533 0 01.345.283.538.538 0 01.022.45z",
  },
  {
    label: "Poke",
    angle: 150,
    color: "#8b8fff",
    logoPath: null, // multi-path — rendered as special case below
  },
];
      ...
      <svg viewBox="0 0 440 440" className="w-full h-full" style={{ filter: "drop-shadow(0 0 30px rgba(132,204,22,0.08))" }}>
        ...
        {tools.map((t) => (
          <g key={t.label}>
            ...
            {/* Logo icon centered inside the circle */}
            {t.label === "Poke" ? (
              /* Poke — multi-path palm tree */
              <g transform={`translate(${t.x - 10}, ${t.y - 10}) scale(0.833)`}>
                <line x1="12" y1="10" x2="12" y2="19" stroke={t.color} strokeWidth="1.4" strokeLinecap="round" />
                <line x1="9" y1="19" x2="15" y2="19" stroke={t.color} strokeWidth="1.4" strokeLinecap="round" />
                <path d="M12 10 Q8 6 5 5" stroke={t.color} strokeWidth="1.2" strokeLinecap="round" fill="none" />
                <path d="M12 10 Q7 7 4 8" stroke={t.color} strokeWidth="1.2" strokeLinecap="round" fill="none" />
                <path d="M12 10 Q8 9 5 11" stroke={t.color} strokeWidth="1.2" strokeLinecap="round" fill="none" />
                <path d="M12 10 Q16 6 19 5" stroke={t.color} strokeWidth="1.2" strokeLinecap="round" fill="none" />
                <path d="M12 10 Q17 7 20 8" stroke={t.color} strokeWidth="1.2" strokeLinecap="round" fill="none" />
                <path d="M12 10 Q16 9 19 11" stroke={t.color} strokeWidth="1.2" strokeLinecap="round" fill="none" />
              </g>
            ) : t.logoPath ? (
              <g transform={`translate(${t.x - 10}, ${t.y - 10}) scale(0.833)`}>
                <path d={t.logoPath} fill={t.color} />
              </g>
            ) : null}
      ...
      <section className="px-8 lg:px-16 py-20" style={{ borderTop: "1px solid #18181B" }}>
        <div className="max-w-4xl">
          ...
          <p className="text-[15px] leading-[1.7] max-w-xl mb-4" style={{ color: "#A1A1AA" }}>
            Anthropic&rsquo;s research found that LLMs maintain a privileged
            subspace &mdash; the Jacobian space &mdash; where only ~25 concepts are
            active at once. Cortex applies this: your AI holds {" "}
            20 memories in its workspace. Unused ones decay. New signals promote fresh ones.
          </p>
          <p className="text-[14px] leading-[1.7] max-w-xl mb-8" style={{ color: "#71717A" }}>
            Memories are scored by relevance, strength, and coherence.
            Seven-day half-life. Eviction below 15% loading.
            Pin, suppress, or release any memory to steer what your AI knows.
          </p>
          <Link
            href="/j-space"
